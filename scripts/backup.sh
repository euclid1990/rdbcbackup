...
get_compress() {
    ...
    gzip)
        ...
        ;;
    bzip2)
        compressUtils=("bzip2" "bzip2" "tbz2")
        ;;
    *)
        if [[ -n "${BACKUP_COMPRESSION}" ]]; then
            echo "Unknown compression requested: $BACKUP_COMPRESSION" 1>&2
            exit 1
        fi
    esac
}
...
backup() {
    ...
    for db in ${databases}; do
        ...
        if [[ -n "${BACKUP_COMPRESSION}" ]]; then
            dumpFilename=${dumpFilename}.${zipExt}
            mysqldump -h ${MYSQL_HOST} -P ${MYSQL_PORT} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} --databases ${db} ${MYSQLDUMP_OPTS} | $zipUtil > ${BACKUP_DIR}/${dumpFilename}
        else
            mysqldump -h ${MYSQL_HOST} -P ${MYSQL_PORT} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} --databases ${db} ${MYSQLDUMP_OPTS} > ${BACKUP_DIR}/${dumpFilename}
        fi

        if [ $? -eq 0 ]; then
            fileSize=`du -h "${BACKUP_DIR}/${dumpFilename}" | cut -f1`
            echo "- Database [${db}] backup successfully completed"
            echo "-- File: ${BACKUP_DIR}/${dumpFilename} ${fileSize}"
        else
