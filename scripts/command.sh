...
if [[ "${BACKUP_LOG_COMBINE}" == "1" || "${BACKUP_LOG_COMBINE}" == "yes" ]]; then
    ...
    echo "${BACKUP_SCHEDULE} /scripts/pipe.sh 2>&1 | /scripts/format.sh >>${BACKUP_OUTPUT_LOGFILE}" > /crontab.conf
else
    # Seperate into two stream: all and error only
    echo "${BACKUP_SCHEDULE} { /scripts/pipe.sh 2>&1 1>&3 3>&- | /scripts/format.sh ${BACKUP_ERROR_LOGFILE} out; } 3>&1 | /scripts/format.sh >> ${BACKUP_OUTPUT_LOGFILE}" > /crontab.conf
fi
